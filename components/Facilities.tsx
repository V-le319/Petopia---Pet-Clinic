  ...
  return (
    ...
    <section id="facilities" className="w-full h-full bg-mainBG/30  px-4 md:px-16 py-8 gap-4 md:gap-6 md:py-12 ">
      ...
      <div className="max-w-6xl mx-auto flex flex-col justify-center items-center gap-6">
        ...
        <div className="w-full grid grid-cols-3 grid-rows-2 gap-4 sm:gap-6 h-[520px]">
  {facilities.map((facility, index) => (
    ...
    >
      ...
      <div className="absolute inset-0 bg-headline/0 group-hover:bg-headline/60 transition-all duration-300 flex items-end p-4">
        <p className="text-white font-medium opacity-0 group-hover:opacity-100 transition-all duration-300 translate-y-2 group-hover:translate-y-0">
          {facility.title}
        </p>
      </div>
    </motion.div>
